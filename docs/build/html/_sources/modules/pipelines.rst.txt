...
This module proposes several workflows that greatly simplify
deep learning research on CT-scans. Each workflow is represented
in a form of several preprocessing operations `actions
<https://github.com/analysiscenter/dataset>`_, chained in a
`pipeline <https://github.com/analysiscenter/dataset>`_.
...
of cancerous crops. We can then chain `pipe` with more actions
for training, say, [DenseNet](linkondense):
...
Alternatively, we can choose to save the prepared dataset of crops
...
Worth noting that the defined pipeline will generate `~1500`
training examples, when run one time through the Luna-dataset
(one epoch). In the same time, it may take a couple of hours to
...
A preparation of a richer training dataset can be achieved in two
steps. During the first step we dump cancerous and non-cancerous
crops on a disk. The procedure goes as follows:
